...
const PUZZLE_LEVELS = [
  ...
  /* ---------- 概念收尾篇：核 · 不变子空间 · 鞍点 · 秩 1 ---------- */
  {
    title: "第 19 关 · 指定零空间",
    desc: "构造一个<b>核恰好是 (1,−1) 方向</b>的矩阵：A(1,−1) = <b>(0,0)</b>（黄色向量被送进原点），但矩阵本身不能是零矩阵。",
    hint: "核 = (1,−1) 方向 ⟹ A(1,−1) = 1·î′ − 1·ĵ′ = 0 ⟹ 两列必须完全相同！比如 î′=(1,2)、ĵ′=(1,2)。",
    success: "指定了核，就指定了「谁会消失」。核方向 (1,−1) ⟹ 两行都必须在 (1,1) 方向上 —— 秩最多 1。核与值域的维数此消彼长（第 23 章）。",
    initM: { a: 1, b: 0, c: 0, d: 1 },
    v: { x: 1, y: -1 },
    star: { x: 0, y: 0 },
    showV: true,
    check: (M) => {
      const r = LA.apply2(M, { x: 1, y: -1 });
      const maxA = Math.max(Math.abs(M.a), Math.abs(M.b), Math.abs(M.c), Math.abs(M.d));
      return LA.v.len(r) < 0.05 && maxA > 0.4;
    },
  },
  {
    title: "第 20 关 · 不变 x 轴",
    desc: "构造一个让 <b>x 轴成为不变子空间</b>的矩阵：A(1,0) 仍在 x 轴上（î′ 的纵坐标为 0），且把 ĵ <b>拉出</b> y 轴（ĵ′ 的横坐标 ≠ 0，证明这不是巧合）。",
    hint: "î′ = (a, c)：c = 0 时 x 轴不变。再把 b 拖成非零，ĵ′ 就被推离 y 轴 —— 这就是<b>上三角矩阵</b> [a, b; 0, d]。",
    success: "上三角矩阵 = <b>x 轴是不变子空间</b>的矩阵。不变子空间是分块（第 15 章）、Jordan（第 25 章）能「拆」矩阵的根本原因：找到一条不变线，就能把矩阵削掉一块。",
    initM: { a: 1, b: 0, c: 0, d: 1 },
    check: (M) => Math.abs(M.c) < 0.05 && Math.abs(M.b) > 0.4,
  },
  {
    title: "第 21 关 · 鞍点制造者",
    desc: "结合第 22 章的迹-det 平面：构造一个<b>鞍点矩阵</b> —— tr = 0 且 det &lt; 0（特征值一正一负，空间在一个方向拉伸、另一个方向压缩）。",
    hint: "tr = 0 ⟹ d = −a；det = −a² − b·c &lt; 0 ⟹ b·c 取正。比如 î′=(0,1)、ĵ′=(1,0)。",
    success: "鞍点：特征值 λ 与 −λ 成对出现 —— 一个方向拉伸 e^λ，垂直方向压缩。迹-det 平面的下半区（第 22 章）全是这样「有得必有失」的变换。",
    initM: { a: 1, b: 0, c: 0, d: 1 },
    check: (M) => Math.abs(LA.trace2(M)) < 0.06 && LA.det2(M) < -0.5,
  },
  {
    title: "第 22 关 · 秩 1：全员上直线",
    desc: "构造一个<b>秩 1 矩阵</b>：î′ = (1,1)，且 ĵ′ 也落在直线 <b>y = x</b> 上 —— 整个平面被压到这条直线上，且 î 恰好被送到 (1,1)。",
    hint: "两列都必须在 y = x 上：î′=(1,1)，ĵ′ 取 (2,2) 或 (−1,−1) 都行 —— 秩 1 矩阵的两列永远平行。",
    success: "秩 1 矩阵 = 「整个平面 → 一条直线」的机器：两列平行（第 23 章的列空间）。它还可以写成 <b>u·vᵀ</b>（一列乘一行）—— 所有秩 1 矩阵都是这么简单。",
    initM: { a: 1, b: 0, c: 0, d: 1 },
    check: (M) => {
      const c1 = { x: M.a, y: M.c }, c2 = { x: M.b, y: M.d };
      const cross = c2.x - c2.y;   // c2 与 (1,1) 的叉积
      return LA.v.dist(c1, { x: 1, y: 1 }) < 0.08 && Math.abs(LA.det2(M)) < 0.05 && Math.abs(cross) < 0.08;
    },
  },
];
...
LA.scenes.push({
  ...
  tagline: "22 道关卡，从送向量回家到指定奇异值与零空间",
